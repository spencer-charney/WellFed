...
import { list, listByUser } from "./components/NewPost/apiPost";
import { isAuthenticated } from "./components/landingPage/Auth";
...
class App extends React.Component {
    ...
    render() {
        const { error, isLoaded, myPosts, discoverPosts } = this.state;
        ...
        if (error) {
            ...
            const followersLength = 0;
            const followingLength = 0;
            if (user.followers) followersLength = user.followers.length;
            if (user.following) followingLength = user.following.length;

            const pageState = this.state.pageState;
            let page;
            if (pageState == 'profile') {
                page = <MyProfile name={user.name} username={user.username} followers={followersLength} following={followingLength} restrictions={user.restrictions} myBooks={[{
                    name: "Kid1", posts: [
