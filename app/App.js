...
class App extends React.Component {
    ...
    render() {
        const { self, error, isLoaded, myPosts, discoverPosts, myFeedPosts } = this.state;
        ...
        if (error) {
            ...
            if (pageState == 'profile') {
                page = <MyProfile self={self} name={self.name} username={self.username} followers={followersLength} following={followingLength} restrictions={self.restrictions} myBooks={[{
                    name: "Kid1", posts: [
                ...
                page = <NotificationsPage notifications={self.notifications}
                // {[
